...
class NepiIFSimControls extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      // Last-synced "environment" Setting value (e.g. "FLAT_GROUND"),
      // write-only bookkeeping now that the dropdown that used to render
      // this has been removed -- kept only so a future reader can see
      // what the driver last reported without adding a new RUI control.
      selected_environment_setting: 'FLAT_GROUND',
  ...
  rbxSettingsListener(message) {
    ...
    if (valuesDict["environment"] !== undefined) {
      ...
      if (pending !== null && valuesDict["environment"] !== pending) {
        ...
        updates.selected_environment_setting = valuesDict["environment"]
      }
    }
        ...
        {(setup_actions.length > 0) ?
          <ButtonMenu>
            {setup_actions.map((action, i) => {
              // RESET_SIM moved to renderRobotCapabilityControls's own
              // dedicated button (requested live 2026-09-14: "the reset_sim
              // command shouldnt be in the setup actions of robots... move
              // it there instead [Sim Connector]... maybe keep it in the
              // robot capabilities section"). `i` is still the right index
              // for every OTHER action below -- this array itself is never
              // filtered, only this render skips one entry.
              if (action === 'RESET_SIM') {
                return null
              }
              return (
  ...
  renderRobotCapabilityControls() {
    ...
    // instant rbx_namespace re-points at a new device (see that method's
    // own comment) -- it only refills once the new device's FIRST
    // SettingsStatus message actually arrives. Treating `live` alone as
    // "the settings list is authoritative" made this whole section vanish
    // for that entire window, since an empty list made every settings.
    // includes(...) check below false -- confirmed live (2026-09-09): "when
    // the image topics spawn in for gazebo too, the robot capabilities
    // section... disappears," which is exactly this race (camera topics and
    // a fresh rbx_namespace show up around the same moment a new sim
    // connects). Gate on the list actually having anything in it, not just
    // on `live` -- an empty list means "haven't heard from this device's
    // Settings yet," the same indeterminate state as before any device
    // connects at all, so it gets the same permissive preview defaults
    // rather than being read as "this driver declares none of these."
    const settingsKnown = live && settings.length > 0
    // Preview mode (not live, or live but its Settings haven't arrived yet):
    // these three toggles are declared identically by every RBX sim driver's
    // CAPABILITY_SETTING_NAMES (confirmed by direct inspection of
    // rbx_sim_node.py, rbx_ardupilot_node.py, rbx_webots_node.py,
    // rbx_webots_quadcopter_node.py), so showing them unconditionally is
    // accurate, not a guess. Image-source curation stays gated on real
    // settings data -- there is no candidate topic list to curate before a
    // real camera exists.
    ...
    // shouldnt be in the setup actions of robots... move it there instead
    // [Sim Connector]... maybe keep it in the robot capabilities section
    // instead for that"). Index resolved against the ACTUAL reported
    // setup_action_options list, not hardcoded -- it sits at a different
    // position per driver (rbx_sim_node.py/rbx_mujoco_node.py/
    // rbx_webots_node.py all report it first, index 0;
    // rbx_ardupilot_node.py/rbx_webots_quadcopter_node.py report it after
          ...
          <ButtonMenu>
            <Button
              onClick={() => {
                ...
                // the sliders in renderMotorControls are deliberately NOT
                // continuously re-synced from status (seeded only when the
                // motor COUNT changes, so a live server echo can't fight an
                // operator's own drag in progress). Reset Sim is exactly
                // the one case that SHOULD override local slider state
                // outright, requested live (2026-09-02): "this change
                // should happen in the RUI too if they are set to
                // something and reset sim is hit" -- reuse turnOffAllMotors
                // (same "snap sliders to 0 and publish it" the All-Off
                // button already does) rather than a near-duplicate method.
  ...
  renderImageSourceCuration() {
    ...
    const candidates = image_topics.filter((topic) => {
      ...
      // robot_depth_map/scene_depth_map (raw float32-meters, for downstream
      // processing) removed entirely 2026-09-14 (requested live: "unness
      // and dont do anything") -- no processing-pipeline consumer ever
      // existed for them, just extra bandwidth and noise in this picker.
  ...
  // Depth map is always available now (no backend gate left to toggle) --
  // the "Depth Map" switch that used to live here is gone; see this
  // component's own render() history for the removal note.

  // Drives the RBX driver's own "environment" Setting -- the ONLY thing
  // that actually spawns/despawns a model in Gazebo (rbx_sim_node.py's
  // setEnvironmentAction -> sim_bridge_node.py ->
  // environment_models.py's real /gazebo/spawn_sdf_model|delete_model
  // calls). Nepi_IF_Sim.js's own "Environment Config" dropdown is a
  // COMPLETELY SEPARATE axis that only edits a model's geometry FIELDS
  // (obstacle positions/sizes), never whether it's actually present in the
  // world -- this is the bridge between the two.
  //
  // No longer rendered as its own dropdown here (requested live
  // 2026-09-04: "the depth map and environment selector... is useless...
  // environment should be live changeable from the top selector too") --
  // this is now ONLY called via ref from Nepi_IF_Sim.js's own
  // onSelectDimensionConfig, for EVERY environment dimensions config name,
  // not just the two ("Flat"/"Obstacle Course") the old dropdown offered.
  // value is a dimensions-config DISPLAY name (e.g. "Aerial Obstacle
  // Course"), translated to the Setting's own UPPER_SNAKE_CASE convention
  // (rbx_sim_node.py's processEnvironmentOptionsLine builds
  // CAP_SETTINGS['environment']['options'] as
  // ["FLAT_GROUND"] + [scanned_model_name.upper() for ...], so this only
  // ever needs to guess right when a config name and a real VM-side model
  // directory were named to match on purpose, which every built-in here
  // was) -- "Flat" is the one special case (FLAT_GROUND is a reserved
  // "nothing spawned" value, not itself a model directory). A guess that
  // doesn't match any real backend option is harmless: updateSetting just
  // gets ignored by the driver's own Discrete-setting validation, the same
  // safety net every other Setting update in this platform already relies
  // on, so an exotic custom-saved name with no VM-side model silently does
  ...
  // more than half the scren for both of them combined."
  //
  // First fix (a fixed 220px per viewer) was sized for the two-column
  // layout that existed that day, where each viewer's own Column was
  // already about a quarter of the page; once the RUI went back to a
