...
class SimDiscovery:
  ...
  def launchSimDeviceNode(self, path_str, robot_slot):
    ...
    # rbx_sim_node.py's `from nepi_api.device_if_rbx import RBXRobotIF` (->
    # nepi_pc -> `import open3d`) crashes with "libgomp.so.1: cannot allocate
    # memory in static TLS block" on this aarch64 build whenever cv2 (also
    # imported by this node, earlier) has already claimed libgomp's one
    # static TLS slot -- confirmed live (2026-09-08) via drivers_mgr's own
    # restart log showing this exact traceback for rbx_sim_node.py, blocking
    # every RBX_SIM-driven simulator (rover and any other non-ArduPilot
    # target) from ever registering a device. Preloading libgomp before the
    # interpreter starts guarantees it gets the only claim regardless of
    # import order. Restored right after the spawn call returns so this
    # doesn't leak into unrelated drivers_mgr spawns that never needed it.
