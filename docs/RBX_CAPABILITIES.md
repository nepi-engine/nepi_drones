...
| Manual motor control | `setMotorControlRatio` (+ `getMotorControlRatios`, `manualControlsReadyFunction`) | `has_manual_controls` | `rbx/set_motor_control` (see [section 11, Motor Control Ratios Deep Dive](#11-motor-control-ratios-deep-dive)) |
...
## 11. Motor Control Ratios Deep Dive

Manual motor control (the `setMotorControlRatio` / `getMotorControlRatios` capability
introduced in sections 4 and 10) is the interface's least autonomous control path: a
client tells the vehicle "motor 2 to 40% speed" and it does exactly that, with no flight
control in between. This section covers how it works end to end: the wire format, the two
driver functions, the command path through `device_if_rbx.py`, how it surfaces in the RUI,
and how to implement it on a new driver.

### 11.1 What "motor control ratios" are
...
- **Purpose**: set one motor to a speed ratio. This is where the driver actually talks to
  ...
  motor at a time.
- Supplying this function (non-`None`) is what turns the whole capability on. See 11.4.
...
### 11.3 The command path (what happens end to end)
...
### 11.4 Wiring it into RBXRobotIF
...
### 11.5 Setting it from the robot's Device page
...
11.3: if `Manual Ready` is false, the command is silently ignored (a warning is logged, no
motion happens).

### 11.6 Implementing the two functions on a new driver
...
### 11.7 Changelog: motor-control bugs fixed 2026-07-16
...
## 12. Building a new RBX driver (and adding capabilities)
...
### 12.2 Enabling or disabling an existing capability
...
### 12.3 Adding a genuinely new capability (interface change)
...
| SITL simulation setup | `docs/DISCOVERY_EXPLAINED.md#appendix-standing-up-ardupilot-sitl` |
| Driver discovery details | `docs/DISCOVERY_EXPLAINED.md` |
